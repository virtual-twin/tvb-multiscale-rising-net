...
def distribute_pathway_gain(config):
    ...
    M1FACIAL_GAIN_FACTOR = float(config.M1FACIAL_GAIN)
    config.M1FACIAL_GAIN = config.PATHWAY_GAIN + M1FACIAL_GAIN_FACTOR * (config.MAX_GAIN - config.PATHWAY_GAIN)
    if config.VERBOSITY:
        print("M1FACIAL_GAIN = %g (= PATHWAY_GAIN + M1FACIAL_GAIN_FACTOR * (MAX_GAIN - PATHWAY_GAIN) = "
              "%g + %g * (%g - %g))" %
              (config.M1FACIAL_GAIN, config.PATHWAY_GAIN, M1FACIAL_GAIN_FACTOR, config.MAX_GAIN, config.PATHWAY_GAIN))
    CNM1S1_GAIN_FACTOR = float(config.CNM1S1_GAIN)
    config.CNM1S1_GAIN = CNM1S1_GAIN_FACTOR * config.PATHWAY_GAIN
    for gain in ["CNM1_GAIN", "CNS1_GAIN"]:
        setattr(config, gain, config.CNM1S1_GAIN)
        if config.VERBOSITY:
            print("%s = %g (= CNM1S1_GAIN_FACTOR * PATHWAY_GAIN = %g * %g)" %
                  (gain, getattr(config, gain), CNM1S1_GAIN_FACTOR, config.PATHWAY_GAIN))
